...
function generatePaginationButtons(currentPage, totalPages, size, searchingValue, tableContent) {
	...
	for (let i = Math.max(2, currentPage - 2); i <= Math.min(currentPage + 2, totalPages - 1); i++) {
		addButton(i, currentPage, size, searchingValue, paginationList, tableContent);
	}
	// thêm gap
...
function addButton(pageNumber, currentPage, size, searchingValue, parentElement, tableContent) {
